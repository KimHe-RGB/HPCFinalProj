...
x0 = 0; xfinal = 1; 
t0 = 0; tfinal = 1; 
err_ie_2s = zeros(length(hxs),1);
err_cn_2s = zeros(length(hxs),1);
...
for i = 1:length(hts)
    ...
    % Solve with explicit solver
    u_cn = Heat1D_CN_solver([x0 hx xfinal],[t0 ht tfinal], ...
    init_cond_f, diffusivity_f, force_f,...
        lbry_f, rbry_f, ...
        leftType, rightType);
    u_ie = Heat1D_IE_solver([x0 hx xfinal],[t0 ht tfinal], ...
        init_cond_f, diffusivity_f, force_f,...
        lbry_f, rbry_f, ...
        leftType, rightType);
    real_u = u_sol_f(xs,tfinal);
    ...
    M = 1024;
    u_ie_int = reshape(interpft(u_ie,M), [M,1]); 
    u_cn_int = reshape(interpft(u_cn,M), [M,1]); 
    real_u_int = reshape(interpft(real_u,M), [M,1]);

    err_ie_2s(i) = norm(u_ie_int - real_u_int, 2);
    err_cn_2s(i) = norm(u_cn_int - real_u_int, 2);
end

% error convergence order
figure(2); clf; 
hold on;
plot(hts, err_ie_2s, 'r-o', DisplayName="$$L_2$$ IE error")
plot(hts, err_cn_2s, 'g-o', DisplayName="$$L_2$$ CN error")
