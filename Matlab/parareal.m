...
%     lbry_f, rbry_f, ...
%     leftType, rightType);
% U(end,:) = coarse_solver_u;

t0 = T0; tfinal = Total_T;
% coarse approximation from i -> i+1
G_coarses = Heat1D_IE_solver_coarse( ...
    [x0 coarse_hx xfinal],[t0 coarse_ht tfinal], U(1,:), ...
    diffusivity_f, force_f,...
    lbry_f, rbry_f, ...
    leftType, rightType, core_num);
for i = 1:core_num-1
    % initial value for i+1 is the ending value of i
    U(i+1,:) = G_coarses(i+1,:);
end
...
for iter = 1:parareal_iters
    ...
    %             [x0 coarse_hx xfinal],[t0 coarse_ht tfinal], F(j,:), ...
    %             diffusivity_f, force_f,...
    %             lbry_f, rbry_f, ...
    %             leftType, rightType);
    ...
    %         U(j+1,:) = G_coarse_new - G(j+1,:) + F(j,:);
    %     end
    G_coarses_new = Heat1D_IE_solver_coarse( ...
            [x0 coarse_hx xfinal],[t0 coarse_ht tfinal], U(1,:), ...
            diffusivity_f, force_f,...
            lbry_f, rbry_f, ...
            leftType, rightType, core_num);
    for i = 1:core_num-1
        % initial value for i+1 is the ending value of i
        U(i+1,:) = G_coarses_new(i+1,:) - U(i,:) + F(i,:);
    end
end
...
function u_outs = Heat1D_IE_solver_coarse(xspan, tspan, ...
    icu, df, F,...
    lbry, rbry, leftBCType, rightBCType, cores)
    ...
    recordEveryNSteps = (tfinal - t0) / ht / cores;
    u_outs = zeros(cores,Nxs); % record the ending value
    count = 1;
    while ~done
        i = i + 1;
        if(t + ht >= tfinal)
