// clang -Xpreprocessor -fopenmp -lomp -I/opt/homebrew/opt/libomp/include/ -o
// parareal -std=c++17 parareal_naive.cpp g++ -std=c++17 -fopenmp -O3
// -I/usr/include/eigen3 parareal.cpp -o parareal
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < range.size(); ++i) {
        ...
        if (rank == 0) {
            for (int i = 0; i < num_subintervals; i++) {
                long double t_s =
                    (i + 1) * (tfinal - t0) / num_subintervals + t0;
                ...
                U_coarse[i + 1] =
                    Heat1D_IE_solver(xspan, tspan_mini, U_coarse[i],
        ...
        do {
            int num_vectors = U_fine.size();
            int num_elements = U_fine[0].size();
    ...
    for (int i = 0; i < range.size(); ++i) {
        std::cout << "iterations: " << range[i] << " hx: " << hx
                  << " ht: " << ht << " T: " << tfinal << '\n';
    ...
    std::ofstream file;
    file.open("solutions.csv");
    file << std::setprecision(15);

    file << "xs,";
    for (int j = 0; j < Nxs; ++j) {
        file << xs[j];
        if (j != Nxs - 1) {  // not the last element
            file << ",";
        }
    }
    file << "\n";
    ...
    // Write the CN solutions
    file << "Fine solver solutions,";
    for (int j = 0; j < Nxs; ++j) {
        file << u_CN[j];
        if (j != Nxs - 1) {  // not the last element
            file << ",";
    ...
    */
    ...
    // Write the final solutions for each number of iterations
    for (int i = 0; i < range.size(); ++i) {
        file << "PR with iter" << range[i] << ",";
        for (int j = 0; j < Nxs; ++j) {
            file << final_U_solutions[i][j];
            if (j != Nxs - 1) {  // not the last element
                file << ",";
            }
        }
